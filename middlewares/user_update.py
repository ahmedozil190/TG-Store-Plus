        ...
        if tg_user and not tg_user.is_bot:
            try:
                async with async_session() as session:
                    ...
                    if not user:
                        # Process Referral Logic in Middleware (to ensure it works even if SubscriptionMiddleware blocks handlers)
                        ...
                        if referral_id and referral_id != user_id:
                            ...
                            # Use a separate session or careful with this one
                            ...
                            if referrer:
                                ...
                                # Notify referrer via bot from data
                                bot = data.get("bot")
                                if bot:
                                    try:
                        ...
                        session.add(user)
                        logger.info(f"Middleware: Created new user {user_id} with referral_by={user.referred_by}")
                    else:
                        # Update if changed
                        changed = False
